...
class HillClimbingFinalTakvimi:
    ...
    def uygun_sınıfları_bul(self, ders):
        ...
        return atanan_sınıflar if toplam_kapasite >= gerekli_kapasite else None
        ...
        for ders in self.dersler:
            ...
            for gun in self.gunler:
                ...
                sert_kisit_var_mi = any(
                    self.ders_kisitlari.loc[ders, diger_ders] == -1000 for diger_ders in dersler_aynı_gun
                )
    ...
    def neighbor(self, timetable):
        yeni_timetable = copy.deepcopy(timetable)
        ders = random.choice(self.dersler)

        yeni_timetable = [g for g in yeni_timetable if g[0] != ders]
        ...
        if uygun_sınıflar:
            ...
            yeni= [(ders, tarih, saat, sinif_ad) for sinif_ad, _ in uygun_sınıflar]
            yeni_timetable.extend(yeni)

        return yeni_timetable
    ...
    def cost(self, timetable):
        ...
        for i in range(len(timetable)):
            ...
            for j in range(i + 1, len(timetable)):
                ...
                kural = self.ders_kisitlari.loc[ders1, ders2]
    ...
    def run(self, iterations=1000):
        ...
        for i in range(iterations):
            ...
            if i % 100 == 0:
                print(f"Iterasyon {i} | Cost: {current_cost}")
...
class FinalTakvimUygulamasi:
    def __init__(self, root):
        ...
        donem_secenekleri = ["Tekrar/Seçmeli"]  # Başlık ekle
                ...
                if sheet_name.lower() != "sınıflar": 
                    ...
                    for i, donem in enumerate(donemler):
                        ...
                        for cell in sheet.iter_rows(min_row=2, min_col=i+1, max_col=i+1):
                            if cell[0].value:  # Boş olmayan hücreleri al
    ...
    def takvim_hazirla(self):
        ...
        hillclimbing = HillClimbingFinalTakvimi(ders_kisitlari, gunler, zaman_dilimleri, self.ders_bolum_sayilari , sınıflar, bolumler)
        best_schedule = hillclimbing.run(5000)
